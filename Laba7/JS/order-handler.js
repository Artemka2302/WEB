...
// ФИКС: Восстанавливаем отображение блюд
function fixDishesDisplay() {
    // Даем время на загрузку
    setTimeout(() => {
        // Находим все сетки блюд
        const dishGrids = document.querySelectorAll('.dishes-grid');
        
        // Проверяем, что они отображаются как grid
        ...
        // Перезагружаем блюда если функция существует
        if (typeof displayDishesByCategory === 'function') {
            displayDishesByCategory('all');
            console.log('Блюда восстановлены!');
        }
    }, 800);
}

// Запускаем фикс при загрузке
document.addEventListener('DOMContentLoaded', fixDishesDisplay);
